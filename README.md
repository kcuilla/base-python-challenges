# base-python-challenges
## Solving difficult problems using base python only and testing them with unittest

The code in this repo attempts to solve three separate problems in python by only using the base python libraries (no pandas, no numpy, etc.). Extensive unit tests are included with each problem to make sure that the code runs successfully and throws erorrs when the user inputs incorrect or unexpected values. 

### Question 1

Write a function that returns all prime numbers from a given list of integers where the max value in the list cannot exceed 101.
...
Write a function that takes an integer between negative one billion and positive one billion, including zero, and returns the english-language equivalent.
...
Write a function that takes a list character sequences, removes all non-alphanumeric characters from these, one-hot encodes the sequences and returns the result.
